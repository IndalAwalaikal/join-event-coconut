...
const AdminPendaftaranOpenClass = () => {
  ...
  const fetchRegistrations = async () => {
    setLoading(true);
    try {
      // ✅ Kirim query parameter dengan benar
      const response = await ApiClient.get<{
        status: string;
        message: string;
        data: any[]; // ← ini yang berisi array registrasi
      }>("/admin/registrations", {
        jenis: "open_class",
      });
  ...
  const handleDelete = async (registrationId: string) => {
    ...
    try {
      // ✅ Kirim body JSON: { jenis: "open_class", id: 123 }
  ...
  const exportToCSV = () => {
    ...
    link.download = `pendaftaran-openclass-${new Date().toLocaleDateString(
      "id-ID"
    )}.csv`;
    link.click();
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-800 mb-2">
              Pendaftaran Open Class
            </h1>
            <p className="text-slate-600">
              Kelola peserta yang mendaftar Open Class
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
            ...
            <p className="text-slate-600">
              {searchQuery
                ...
                : "Belum ada peserta yang mendaftar Open Class"}
            </p>
          </div>
        )}
      </div>
...
export default AdminPendaftaranOpenClass;
